...
def xsam_collate_fn(
    ...
    has_conversation = any(inst.get("conversation") is not None for inst in instances)
    has_task_category = any(inst.get("task_category") is not None for inst in instances)
    ...
    for example in instances:
        ...
        if has_mask_label:
            mask_labels.append(example.get("mask_labels"))
        if has_class_label:
            class_labels.append(example.get("class_labels"))
        if has_sampled_labels:
            sampled_labels.append(example.get("sampled_labels"))
        if has_contiguous_labels:
            contiguous_labels.append(example.get("contiguous_labels"))
        if has_conversation:
            conversations.append(example.get("conversation"))
        if has_task_category:
            task_categories.append(example.get("task_category"))
    ...
    if has_conversation:
        data_samples.set_metainfo({"conversations": conversations})
    
    if has_task_category:
        data_samples.set_metainfo({"task_categories": task_categories})
