...
class XSamModel(BaseModel):
    ...
    def predict(self, data_dict, data_samples=None, **kwargs):
        ...
        if llm_outputs is not None:
            ...
            elif seg_ids is not None and (seg_ids >= 0).any():
                # 当 generate() 仅返回生成 token（使用 inputs_embeds 时）而非 input+generated 时，
                # llm_output_ids 不含输入序列，需使用 prepare_inputs_labels_for_multimodal 已扩展的
                # seg_ids（已映射到 inputs_embeds 空间）来定位输入问题中的 <SEG> hidden state。
                seg_embeds = self._get_index_embeds(llm_input_embeds, seg_ids)
            else:
