...
class XSamModel(BaseModel):
    def __init__(
        ...
        # Initialize token indices to -1 by default
        ...
        if self.use_llm_lora:
            # 这一部分有修改，为了适配新增specical tokens的lora训练
            llm_lora = deepcopy(llm_lora)

            # 只训练新增 special tokens 的输入 embedding
            ...
            print_log(f"trainable_ids: {trainable_ids}", logger="current")

            if isinstance(llm_lora, dict):
                llm_lora["trainable_token_indices"] = trainable_ids
        ...
        if s1_pretrained_pth is not None:
            ...
            for key, value in pretrained_state_dict.items():
                if 'class_predictor' in key:
                    ...
                    print_log(f"Skipping {key} due to potential class number mismatch", logger="current")
                    continue
                else:
                    filtered_pretrained_dict[key] = value
            
            self.load_state_dict(filtered_pretrained_dict, strict=False)

            matched_keys = [k for k in filtered_pretrained_dict.keys() if k in state_dict.keys()]
        ...
        if s2_pretrained_pth is not None:
            ...
            print_log(f"matched keys: {matched_keys}", logger="current")
        ...
        out_mod = self.llm.get_output_embeddings()
        print(type(out_mod))
        if out_mod is not None and hasattr(out_mod, "weight"):
            ...
            print("No explicit output embeddings / lm_head found")
        inp = self.llm.get_input_embeddings().weight
        out = self.llm.get_output_embeddings().weight

        print("embed requires_grad:", inp.requires_grad)
        print("lm_head requires_grad:", out.requires_grad)

        for n, p in self.named_parameters():
            if "embed_tokens" in n.lower() or "lm_head" in n.lower():
                print(n, p.requires_grad, tuple(p.shape))
        self.state_dict()
    ...
    def _add_special_tokens(self, special_tokens):
        ...
        if num_new_tokens > 0:
            # 打印新增token之前embedding及llm_head的形状
            print_log(f"Embedding shape before adding tokens: {self.llm.get_input_embeddings().weight.shape}", logger="current")
            out_mod = self.llm.get_output_embeddings()
            if out_mod is not None and hasattr(out_mod, "weight"):
                print_log(f"LM head shape before adding tokens: {out_mod.weight.shape}", logger="current")
            print_log(f"Added {num_new_tokens} special tokens: {special_tokens}", logger="current")
            self.llm.resize_token_embeddings(len(self.tokenizer))
            # 打印新增token之后embedding及llm_head的形状
            print_log(f"Embedding shape after adding tokens: {self.llm.get_input_embeddings().weight.shape}", logger="current")
            out_mod = self.llm.get_output_embeddings()
            if out_mod is not None and hasattr(out_mod, "weight"):
                print_log(f"LM head shape after adding tokens: {out_mod.weight.shape}", logger="current")
    ...
    def _forward(
        ...
        **kwargs,
        ...
        #print(f"🔍 _forward中获取的seg_image_embeddings: {[feat.shape for feat in seg_image_embeddings] if seg_image_embeddings is not None else None}")
        ...
        # Allow mixed tasks in batch for multi-task training
        assert all(
            task_name in DEFAULT_TASKS for task_name in set(task_names)
        ), f"Task names {set(task_names)} are not all in {DEFAULT_TASKS}"
        ...
        if (local_cond_lens or global_cond_lens) is not None and mask_labels is not None:
            ...
            # For refseg/reaseg, class_labels are instance indices in [0, num_classes) and must not be offset.
            # For other tasks (e.g. genseg/ovseg), class_labels are offset by condition length for the matcher.
            if task_names[0] in ("refseg", "reaseg"):
                class_labels = list(chain(*[class_label.split(1) for class_label in class_labels]))
            else:
        ...
        if seg_embeds is not None or llm_outputs is None:
            if seg_embeds is not None and seg_embeds.shape[1] != 1:
                ...
                #print(f"🔍 _forward中传递给segmentor的image_embeddings: {[feat.shape for feat in seg_image_embeddings] if seg_image_embeddings is not None else None}")
    ...
    def predict(self, data_dict, data_samples=None, **kwargs):
        """
        推理时 mask 触发条件（与训练对齐：训练/推理都采用问题里的 <SEG>）：
        1) 若**问题/input**里已有 <SEG>：用该位置的 LLM hidden 做 seg_embeds（与训练一致）。
        2) 若**回答**里出现 <SEG>：用回答中 <SEG> 位置的 hidden 做 seg_embeds（兼容旧 prompt）。
        3) 若任务属于 SEG_REQUIRED_TASKS 但前后都没有 <SEG>：用词表 <SEG> 的 embedding 经 llm_projector 兜底。
        4) imgconv 等非分割任务不会生成 mask。
        分割任务的数据模板已在 question 末尾加 <SEG>、answer 不含 <SEG>，训练与推理一致。
        """
        if data_dict.get("inputs_embeds", None) is not None:
            data_dict["input_ids"] = None
        ...
        if llm_outputs is not None:
            ...
            # seg_embeds 优先用问题里的 <SEG>（信息在提问里已明确，不依赖回答是否输出 <SEG>），其次用回答里的 <SEG>，最后词表兜底
            input_sequence = llm_output_ids[:, :L]
            seg_idx_in_input = (input_sequence == self.seg_token_idx).nonzero()[:, 1]
            if len(seg_idx_in_input) > 0:
                seg_ids_input = torch.full(
                    (input_sequence.shape[0], L), -1, dtype=torch.long, device=input_hidden_states.device
                )
                for i, idx in enumerate(seg_idx_in_input):
                    seg_ids_input[:, idx] = i
                seg_embeds = self._get_index_embeds(llm_input_embeds, seg_ids_input)
            else:
                ...
                if len(seg_idx) > 0:
                    ...
                    cond_embeds is not None
                    and task_names[0] in SEG_REQUIRED_TASKS
                    and self.seg_token_idx >= 0
                ):
                    device = input_hidden_states.device
                    seg_token_raw = self.llm.get_input_embeddings()(
                        torch.tensor([[self.seg_token_idx]], dtype=torch.long, device=device)
                    )
                    seg_embeds = [self.llm_projector(seg_token_raw).squeeze(0)]
    ...
    def compute_loss(self, data_dict, data_samples=None, **kwargs):
        ...
        # 用于在未计算的分支上占位 0，保证与 loss 同 device，便于 MMEngine 滑动平均后 loss ≈ loss_llm + loss_seg
        device = next(self.parameters()).device
        zero = torch.tensor(0.0, device=device, dtype=torch.float32)

        loss, loss_llm, loss_seg = zero.clone(), zero.clone(), zero.clone()
    ...
    def state_dict(self, *args, **kwargs):
        ...
        # Step 8. trainable_tokens_delta
        to_return.update({k: v for k, v in state_dict.items() if "trainable_tokens_delta." in k})
        print_log("is trainable_tokens_delta exist: {}".format(any("trainable_tokens_delta" in k for k in to_return.keys())), "current")
        return to_return
    ...
    def _prepare_llm_for_lora(self, lora_config, use_activation_checkpointing=True):
        ...
        # 注释以优化内存的使用，目前使用的不是 4/8 bit 量化模型，无需 prepare_model_for_kbit_training 的处理，如果额外调用会造成内存激增；如果后续增加了 4/8 bit 量化的模型，再取消注释并调整相关代码即可。
