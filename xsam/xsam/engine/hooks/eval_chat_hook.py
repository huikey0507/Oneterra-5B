...
class EvaluateChatHook(Hook):
    ...
    def _eval_images(self, runner, model, device, max_new_tokens=None, save_eval_output=False):
        ...
        for (
            ...
            image = expand2square(
                sample_image,
                tuple(int(x * 255) for x in image_mean),
            )
            # 使用实际的图像处理器处理图像
            if hasattr(actual_image_processor, 'preprocess'):
                pixel_values = actual_image_processor.preprocess(image, return_tensors="pt")["pixel_values"][0]
            else:
                pixel_values = actual_image_processor(image, return_tensors="pt")["pixel_values"][0]
