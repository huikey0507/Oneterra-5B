...
def evaluate_dataset(
    ...
    for data in tqdm(dataloader, desc=f"Evaluating {data_name}", disable=rank != 0):
        ...
        if visualizer is not None and 'imgconv' not in data_name:
            # Draw predictions
            ...
            scaled_size = data["data_samples"].scaled_sizes[0]
            sampled_labels_batch = getattr(data["data_samples"], "sampled_labels", None)

            for i, (image_info, segmentation_output, mask_gt, class_gt) in enumerate(zip(image_infos, seg_outputs, mask_labels, class_labels)):
...
def main():
    ...
    for dataset_cfg, evaluator_cfg in zip(cfg.val_datasets, cfg.val_evaluators):
        try:
            ...
            evaluate_dataset(model, dataset, evaluator, rank, world_size, generation_config, stop_criteria, visualizer)
        except Exception as e:
