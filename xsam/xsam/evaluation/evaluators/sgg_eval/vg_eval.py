...
def do_vg_evaluation(
    ...
    # PredCls (Predicate Classification): 给定真实的物体边界框和类别，仅要求模型预测物体间的关系。

    # SGCls (Scene Graph Classification): 给定真实的物体边界框，要求模型预测物体的类别以及它们之间的关系。

    # SGDet (Scene Graph Detection): 从头开始，要求模型同时检测物体位置、识别物体类别并预测关系。这是最难的任务。 
    iou_types = ["relations"]
    ...
    if "relations" in iou_types:
        ...
        for groundtruth, prediction in zip(groundtruths, predictions):
            if len(groundtruth['gt_bboxes'])==0 or len(prediction['pred_bboxes'])==0 \
                ...
                print("Warning: empty gt or prediction, skip this image.")
                print("len(groundtruth['gt_bboxes']) = {}, len(prediction['pred_bboxes']) = {}, len(groundtruth['gt_triplet']) = {}, len(prediction['pred_triplet']) = {}".format(
                    len(groundtruth['gt_bboxes']), len(prediction['pred_bboxes']), len(groundtruth['gt_triplet']), len(prediction['pred_triplet'])
                )) 
                continue
        ...
        if len(result_dict[mode + '_recall'][1000]) == 0:
            result_dict[mode + '_recall'][1000] = [0.0]

        result['recall'] = float(np.mean(result_dict[mode + '_recall'][1000]))
        result['mean_recall'] = float(result_dict[mode + '_mean_recall'][1000])
