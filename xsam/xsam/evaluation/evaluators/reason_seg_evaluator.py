...
class ReasonSegEvaluator(ReferSegEvaluator):
    ...
    def _eval_predictions(self, predictions, gt_json):
        ...
        for pred in predictions:
            ...
            ignore_mask = decode_mask(ignore_mask, height, width).astype(np.int32)
            gt_mask = decode_mask(gt_mask, height, width).astype(np.int32)
        ...
        for i, cat_name in enumerate(self.iou_stat.cat_names):
            res[cat_name] = {
                "cIoU": self.iou_stat.ciou[i],
                "gIoU": self.iou_stat.giou[i],
            }
        if self._output_dir:
