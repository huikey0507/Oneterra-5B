...
# X-SAM SAR Align Stage B2 (cond_adapter) 评测
# 加载链: S3(config) -> Stage A (prev, projectors) -> B2 (pth, connector+cond_adapter+router)
# 不改动 xsam_eval_sar_align.sh（仍默认评 Stage B）。
...
CONFIG="./xsam/xsam/configs/xsam/phi3_mini_4k_instruct_siglip2_so400m_p14_384/s3_mixed_fineture_base/xsam_dev_sar_align_B2_eval.py"
DEFAULT_PTH_MODEL="/mnt_llm_A100_V1/shui/LAE/OneTerra-train/wkdrs_sar_align_B2_cond/iter_11456.pth"
...
WORK_DIR="${2:-./eval_results_sar_align_stageB2_cond}"
...
echo "=========================================="
echo "X-SAM SAR Align Stage B2 (cond_adapter) 评估"
echo "=========================================="
echo "配置文件:       $CONFIG"
echo "Checkpoint:     $PTH_MODEL"
echo "Stage A (prev): $PREV_SAR_A_CKPT"
if [ "${1:-}" = "" ]; then
    echo "  (使用默认 B2 checkpoint)"
fi
echo "输出目录:       $WORK_DIR"
